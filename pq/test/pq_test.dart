...
void main() {
  group('draft-ietf-hpke-pq MLKEM768-X25519 / HKDF-SHA256 / ChaCha20Poly1305', () {
    final v =
        (jsonDecode(File('test/fixtures/hpke_pq_mlkem768_x25519.json').readAsStringSync())
    ...
    test('Encap with ikmE as randomness reproduces enc and shared_secret', () {
      final e = MlKem768X25519.encapsulate(hex(f('pkRm')), randomness: hex(f('ikmE')));
    ...
    test('key schedule and first encryption', () async {
      ...
      final e0 = (v['encryptions']! as List<Object?>).first! as Map<String, Object?>;
  ...
  group('Appendix A post-quantum vector', () {
    ...
    final vector = ((fixture['vectors']! as Map<String, Object?>)['direct-hpke-base-pq'])!
        as Map<String, Object?>;

    PrivateVid priv(String n) => PrivateVid(
      id: ids[n]!['id']! as String,
      signingKey: MlDsa65SigningKey(b64(ids[n]!['skS']! as String)),
      decryptionKey: MlKem768X25519DecryptionKey.fromSeed(b64(ids[n]!['skE']! as String)),
    );
    PublicVid pub(String n) => PublicVid(
      id: ids[n]!['id']! as String,
      verificationKey: MlDsa65VerificationKey(b64(ids[n]!['pkS']! as String)),
      encryptionKey: MlKem768X25519EncryptionKey(b64(ids[n]!['pkE']! as String)),
    );
      ...
      await expectLater(
        Tsp.open(
          ...
          receiver: PrivateVid(
            ...
            decryptionKey: X25519DecryptionKey.fromSecret(Uint8List(32)..[0] = 9),
          ),
          sender: pub('pq_alice'),
        ),
    ...
    test('pack is deterministic with fixed randomness and round-trips', () async {
      ...
      final m = await Tsp.open(a.bytes, receiver: priv('pq_bob'), sender: pub('pq_alice'));
    ...
    test('SsiVidResolver maps the post-quantum did:peer:4 documents', () async {
      ...
      final vid = await resolver.resolve(ids['pq_alice']!['longForm']! as String);
      expect(vid.verificationKey, isA<MlDsa65VerificationKey>());
      expect(vid.verificationKey.bytes, b64(ids['pq_alice']!['pkS']! as String));
